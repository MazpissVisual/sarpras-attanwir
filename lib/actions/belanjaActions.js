...
// Admin client pakai service role key (bypass RLS untuk operasi server-side)
...
export async function createBelanjaTransaction(formData) {
  try {
    ...
    const { data, error } = await adminClient.rpc('proses_belanja_baru', {
      ...
      p_user_id: null, // Tidak pakai auth context di server action ini, bisa diupdate nanti
    });
